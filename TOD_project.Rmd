...
### Population
If TOD is a desirable trait in a neighborhood, then transit-rich neighborhoods should see an increase in population over time. However, since the TOD area where transit lines intersect also tend to be the city downtown area. Transit may not be the reason behind the population boost.
...
ggplot(allTracts.group)+
  ...
  labs(title = "Total Population 2010&2018", subtitle = "Boston", caption="Figure 2.1") +
  facet_wrap(~year)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```

For share of adults with bachelor's degrees, it is expected that the more desirable a neighborhood is, the higher this number will be. However, it could also be that 
...
ggplot(allTracts.group)+
  ...
  labs(title = "Share of Adults with Bachelor's Degree 2010-2018", subtitle = "Boston", caption="Figure 2.2") +
  facet_wrap(~year)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```

For median household income in this area, it's expected that households living within the TOD area will see a higher growth in income than those who live in the non-TOD area. However, it could also be the result of the multiplier effect brought by the downtown development.
...
ggplot(allTracts.group)+
  ...
  labs(title = "Median Household Income 2010-2018", subtitle = "Boston", caption="Figure 2.3") +
  facet_wrap(~year)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```

people owning less vehicles due to the developed subway system? This variable seems to have less to do with the subway development to me because the subway was built long ago and the effect it had on vehicles should be much smaller now. Should we change another variable?
...
ggplot(allTracts.group)+
  ...
  labs(title = "Share of Households without a Vehicle 2010-2018", subtitle = "Percentage", caption="Figure 2.4") +
  facet_wrap(~year)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```
...
allTracts.Summary %>%
  ...
  ggplot(aes(year, Value, fill = TOD)) +
    ...
    labs(title = "Indicator differences across time and space", caption="Figure 3.1") +
...
```{r graduated symbol maps, include = True}
...
ggplot() + 
  ...
  facet_wrap(~year)+
  labs(title = "Population 2010-2018", subtitle = "Boston, MA", caption="Figure 5.1") +
  mapTheme() + 
  theme(plot.title = element_text(size=22))
...
ggplot() + 
  ...
  facet_wrap(~year)+
  labs(title = "Rent 2010-2018", subtitle = "Boston, MA", caption="Figure 5.2") +
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```
...
```{r geom_line, include=True}
multiBuffers <- multipleRingBuffer(bosStations, 4, 0.4)
...
ggplot(rent_dis10, aes(x=distance, y=meanrent)) +
  ...
  geom_point()+
  labs(title = "Change of Rent with Distance from Stations, 2010", subtitle = "Boston, MA", caption="Figure 6.1") +
  scale_color_brewer(palette="Paired")+theme_minimal()
  
ggplot(rent_dis18, aes(x=distance, y=meanrent)) +
  geom_line(arrow = arrow())+
  geom_point()+
  labs(title = "Change of Rent with Distance from Stations, 2018", subtitle = "Boston, MA", caption="Figure 6.2") +
  scale_color_brewer(palette="Paired")+theme_minimal()
```
...
ggplot() +   
 # geom_sf(data=st_union(allTractsBos)) +
  ...
  labs(title="Crime Relation with Transit Access, 2018", subtitle="Boston, MA", caption="Figure 7.1") +
#  geom_sf(data = buffer, fill = "transparent", color = "red") +
  mapTheme()
...
ggplot() +   
  ...
  scale_fill_manual(values = palette5,
                    ...
                    name = "Rent\n(Quintile Breaks)") +
  facet_wrap(~TOD) +
  labs(title="Crime Relation with Transit Access, 2012", subtitle="Boston, MA", caption="Figure 7.2") +
  mapTheme()
...
ggplot() +   
  ...
  labs(title="Crime Relation with Transit Access, 2012&2018", subtitle="Boston, MA", caption="Figure 2.5") +
  facet_wrap(~TOD+year) +
  mapTheme() 
